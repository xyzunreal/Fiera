...
int main()
{
	...
	tensor_t<float> temp_in(5,5,1);
	
	for(int i=0; i<5; i++){
		for(int j=0; j<5; j++){
			int maxval = 25;
			temp_in(i,j,0) = 2.19722f / maxval * (rand()-rand()) / float( RAND_MAX );
		}
	}
	...
	for(int i=0; i<5; i++){
		for(int j=0; j<5; j++){
			temp_in(i,j,0) = pow(-1,i^j)*2+i+j-4;
		}
	}
	...
	fc_layer_t * layer4 = new fc_layer_t({2,2,1}, 3);
	layer4->activate(layer3->out);
	fc_layer_bin_t * layer5 = new fc_layer_bin_t({3,1,1}, 2);
